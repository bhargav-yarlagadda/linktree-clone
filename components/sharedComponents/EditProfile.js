...
const EditProfile = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.username && formData.username.includes(" ")) {
            setModalData({
                isOpen: true,
                message: "Username cannot contain spaces. Please choose a different one.",
                type: "error",
            });
            return;
        }
        ...
        try {
            ...
            if (isUsernameTaken) {
                ...
                // Proceed with profile update by making a POST request to /api/profile
                try {
                    ...
                    const updateResponse = await fetch("/api/profile", {
                        ...
                        body: JSON.stringify({
                            email: user?.primaryEmailAddress?.emailAddress, // Assuming you have `user` data
                            username: formData.username,
                            bio: formData.bio,
                            profilePicture: formData.profilePicture, // URL or base64 image
                            backgroundImage: formData.backgroundImage, // URL or base64 image
                        }),
                    });
                    ...
                    if (updateResponse.ok) {
                        ...
                        setModalData({
                            isOpen: true,
                            message:  "Unable to update profile  .test. Please try again later.",
                            type: "error",
                        });
                    }
                } catch (error) {
    ...
    return (
        <div className="max-w-lg w-full rounded-2xl p-4 shadow-input bg-white dark:bg-black">
